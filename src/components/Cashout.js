    ...
    const cashoutSubmit = (e) => {
        ...
        auth.onAuthStateChanged(user => {
            if (user) {
                ...
                db.collection('Buyer-info').doc(user.uid).collection('Information').doc('_'+time).set(
                {
                    BuyerName: name,
                    BuyerEmail: email,
                    BuyerCell: cell,
                ...
                {shoppingCart.map(c => (
                db.collection('Buyer-info').doc(user.uid).collection('Information').doc('_'+time).collection('ProductName').add({
                ...
                <form autoComplete="off" className='form-group' onSubmit={cashoutSubmit}>
                    ...
                    {shoppingCart.map(cartt => (
                        <div>
                            ...
                            <input type="productname" className='form-control' required
                                value={cartt.ProductName} disabled />
                            <input type="quantity" className='form-control' required
                                value={cartt.qty} disabled />
                        </div>
                    ))
                    }
                    <br />
